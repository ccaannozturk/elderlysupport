...
**Database:** Firestore — `matches_v2`, `players_v2`, `locations`,
...
`functions/index.js` keeps its own copy of the Elo, streak, and nemesis
engines, because Cloud Functions only upload the `functions/` directory and
cannot `require()` a file outside it. `tests/elo-parity.test.js` runs both
against the same dataset and fails if a single rating, streak, or nemesis
diverges.
...
- **Public data export** — `public-data/league.json`, generated by
  ...
  Elo/chemistry reimplementation risk). Refreshed event-driven on match save
  via Firestore trigger → GitHub repository dispatch, with a 6-hourly cron
  fallback and manual trigger. See `docs/PUBLIC-DATA.md`.
...
| `onMatchWritten` | — (Firestore trigger) | Event-driven trigger to refresh public data |
...
```bash
# Elo, streak, and nemesis parity: stats-core.js vs. the functions/index.js copy
...
Both currently pass (65 rules assertions, full Elo/streak/nemesis agreement
across every player).
