...
## 4. Roast Studio fixture paste — no AI, drops teams, keeps role markers
...
3. Item 4 (fixture paste / 3-team support) — largest of the four. The
   parsing fixes (route through `parseLineup`, add `(R)`) are worth doing
   and verifying on their own before starting the 3-squad data-shape work,
   since they're independently useful and much lower risk.
4. Item 3 (freshness) last, and only after agreeing the option with the
